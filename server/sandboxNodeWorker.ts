/** Sandbox worker (Node twin of src/agent/sandboxWorker.ts): executes
 *  agent/user JavaScript against a mirrored SceneDocument in a
 *  worker_thread. No DOM, no network imports — only the `api` object and
 *  standard JS. The result document is posted back atomically. */
import { parentPort } from "node:worker_threads";
import { createScriptTarget, createScriptingAPI } from "../src/core/scripting";
import { cloneDoc, type SceneDocument } from "../src/core/types";

interface ExecMessage {
  type: "exec";
  id: number;
  code: string;
  doc: SceneDocument;
}

// Heuristic denylist: the worker has no DOM, but dynamic import/fetch would
// still allow network access. Reject the obvious tokens up front.
const FORBIDDEN = [
  /\bimport\s*[(\s]/, // dynamic import(...) — static imports are syntax errors in new Function anyway
  /\bfetch\s*\(/,
  /XMLHttpRequest/,
  /WebSocket/,
  /\bnew\s+Worker\b/,
  /importScripts/,
  /\brequire\s*\(/,
  /\bprocess\s*\./, // Node twin: block process access the browser never had
  /navigator\s*\./,
  /location\s*\./,
];

function fmt(v: unknown): string {
  if (typeof v === "string") return v;
  try {
    return JSON.stringify(v) ?? String(v);
  } catch {
    return String(v);
  }
}

const port = parentPort;
if (!port) throw new Error("sandboxNodeWorker must run as a worker_thread");

port.on("message", async (e: ExecMessage) => {
  const { type, id, code, doc } = e;
  if (type !== "exec") return;

  const logs: string[] = [];
  try {
    for (const re of FORBIDDEN) {
      if (re.test(code)) {
        throw new Error(`Forbidden API in sandbox code (pattern ${re}) — no network/DOM/imports allowed.`);
      }
    }
    const mirror = cloneDoc(doc);
    const target = createScriptTarget(mirror);
    const api = createScriptingAPI(target, (...args: unknown[]) => {
      logs.push(args.map(fmt).join(" "));
      if (logs.length > 200) logs.length = 200;
    });
    const fn = new Function("api", `"use strict";\nreturn (async () => {\n${code}\n})();`);
    const result = await fn(api);
    port.postMessage({ type: "done", id, ok: true, doc: mirror, logs, result: fmt(result) });
  } catch (err) {
    port.postMessage({
      type: "done",
      id,
      ok: false,
      error: err instanceof Error ? `${err.name}: ${err.message}` : String(err),
      logs,
    });
  }
});
